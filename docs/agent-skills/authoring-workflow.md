...
- Run `hugo server` on `main` for a production-like preview (no drafts).
- Run `hugo --minify` for a production build before committing.

## Staging vs production workflow

- Default to the **staging** worktree for all new drafts and edits (`../personal-site-staging`).
- Compare staging edits to production with `git diff main..staging -- content/writing/<post>.md`.
- When asked to publish, merge `staging` → `main`, set `draft = false`, and commit.
